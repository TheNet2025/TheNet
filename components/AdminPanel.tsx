...
interface AdminPanelProps {
  navigateTo: (page: Page, options?: { userId?: string }) => void;
}
                    ...
                    {activeTab === 'users' && (
                        <Card>
                            <h2 className="font-bold text-xl mb-4 text-text-dark">User Management ({allUsers.length})</h2>
                             <div className="space-y-3 max-h-[60vh] overflow-y-auto pr-2">
                                {allUsers.map(user => (
                                    <div key={user.id} onClick={() => navigateTo(Page.AdminUserDetail, { userId: user.id })} className="flex items-center space-x-4 bg-secondary/80 p-3 rounded-lg cursor-pointer hover:bg-white/10 transition-colors">
